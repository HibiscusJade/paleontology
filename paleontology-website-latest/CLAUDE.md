...
- **Authentication**: register/login/logout/delete-account against a mock user DB persisted in `localStorage` under `paleo_user_db`. Demo accounts exist (see `MOCK_USER_DB`). Login/register dialog is `LoginJoinDialog.tsx`. Logout clears all per-user state (including `userType` and `membershipChoiceMade`) and redirects to home. Available from the top-nav user dropdown ("安全退出") and the PersonalCenter profile tab. Account deletion ("注销账号") permanently removes the user from `paleo_user_db`, clears all per-user localStorage keys, and redirects to home — available in PersonalCenter with a two-step confirmation guard.
